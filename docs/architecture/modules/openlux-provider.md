...
`https://api.openlux.ai/v1`。同一服务商配置支持 Gemini 和 GPT Image；内部自动切换
`/v1beta/models` 与 `/v1/images` 路径。自定义网关的路径前缀会保留。
...
`openlux-gpt-image-edit.ts`。没有将脚本中的关闭安全设置选项设为默认值。
...
参考图上传、响应解析、错误归属和价格覆盖，不消耗真实 API 额度。
